...
export class SubjectHelperService {
    ...
    /**
     * Get subject name by ID from cache (with i18n support)
     */
    getSubjectNameById(subjectId: string): string {
        ...
        const name = currentLang === 'vi' 
            ? (subject.nameVi || subject.nameEn || subject.name || `Subject ${subjectId}`)
            : (subject.nameEn || subject.nameVi || subject.name || `Subject ${subjectId}`);
