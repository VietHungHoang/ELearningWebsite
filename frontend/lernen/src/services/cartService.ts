...
function transformCartItem(item: CartItemResponse): FrontendCartItem {
  ...
  return {
    ...
    instructor: item.instructor || { id: fallbackTutorId, name: fallbackTutor },
    tutor: item.instructor?.name || fallbackTutor,
    tutorId: item.instructor?.id || (typeof item.instructorId === 'number' ? item.instructorId : fallbackTutorId),
    price: item.price || 0,
