...
const Header: React.FC = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false); // TODO: Replace with actual auth state from context/store
          ...
          <div className="flex items-center space-x-4">
            ...
            {!isLoggedIn ? (
              ...
              <div ref={profileRef} className="relative">
                ...
                {/* Test button to toggle logout */}
                <button
                  onClick={() => setIsLoggedIn(false)}
                  className="ml-2 px-3 py-1 text-xs bg-red-200 text-red-600 rounded hover:bg-red-300 transition-colors"
                >
                  Test Logout
                </button>
              </div>
...
const LnNavLink: React.FC<{ to: string; children: React.ReactNode }> = ({
  to,
  children,
}) => (
  <NavLink
    ...
    className={({ isActive }) => `
      ...
      ${
        isActive
          ? "border-[#0b6459]"
          : "border-transparent hover:border-[#0b6459]"
      }
    `}
  >
    {children}
