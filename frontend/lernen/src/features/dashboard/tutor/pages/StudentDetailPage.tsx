...
const StudentDetailPage: React.FC = () => {
    ...
    const [tutorNotes, setTutorNotes] = useState(t('dashboard.tutor.studentDetail.notes.defaultNote'));
    ...
    if (!student) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('dashboard.tutor.studentDetail.notFound.title')}</h2>
                <button
                ...
                >
                    {t('dashboard.tutor.studentDetail.notFound.backButton')}
                </button>
            </div>
        );
    }
                            ...
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                ...
                                <div className="flex gap-2">
                                    <button className="flex items-center gap-2 px-4 py-2 bg-[#0b6459] text-white rounded-lg hover:bg-[#084c43] transition-colors text-sm font-semibold shadow-sm">
                                        ...
                                        {t('dashboard.tutor.studentDetail.actions.message')}
                                    </button>
                                    <button className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm font-semibold">
                                        ...
                                        {t('dashboard.tutor.studentDetail.actions.schedule')}
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <div className="flex flex-col lg:flex-row">
                    ...
                    <div className="w-full lg:w-64 flex-shrink-0 p-4 border-r border-gray-100 lg:min-h-[600px]">
                        <nav className="space-y-2">
                            <NavItem tab="overview" icon={<HiAcademicCap className="w-5 h-5" />} label={t('dashboard.tutor.studentDetail.tabs.overview.label')} description={t('dashboard.tutor.studentDetail.tabs.overview.description')} />
                            <NavItem tab="schedule" icon={<HiCalendar className="w-5 h-5" />} label={t('dashboard.tutor.studentDetail.tabs.schedule.label')} description={t('dashboard.tutor.studentDetail.tabs.schedule.description')} />
                            <NavItem tab="progress" icon={<HiTrendingUp className="w-5 h-5" />} label={t('dashboard.tutor.studentDetail.tabs.progress.label')} description={t('dashboard.tutor.studentDetail.tabs.progress.description')} />
                            <NavItem tab="notes" icon={<HiAnnotation className="w-5 h-5" />} label={t('dashboard.tutor.studentDetail.tabs.notes.label')} description={t('dashboard.tutor.studentDetail.tabs.notes.description')} />
                        </nav>
                    </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                        <StatCard
                                            ...
                                            label={t('dashboard.tutor.studentDetail.overview.stats.sessionsCompleted')}
                                            value={`${student.stats.sessionsCompleted}/${student.stats.totalSessions}`}
                                            color="bg-green-100"
                                            subtext={t('dashboard.tutor.studentDetail.overview.stats.complete', { percent: student.stats.completionRate })}
                                        />
                                        <StatCard
                                            icon={<HiClock className="w-5 h-5 text-orange-600" />}
                                            label={t('dashboard.tutor.studentDetail.overview.stats.sessionsRemaining')}
                                            value={student.stats.sessionsRemaining}
                                            color="bg-orange-100"
                                        />
                                        <StatCard
                                            icon={<HiCheckCircle className="w-5 h-5 text-blue-600" />}
                                            label={t('dashboard.tutor.studentDetail.overview.stats.attendanceRate')}
                                            value={`${student.stats.attendanceRate}%`}
                                            color="bg-blue-100"
                                        />
                                        <StatCard
                                            icon={<HiCalendar className="w-5 h-5 text-purple-600" />}
                                            label={t('dashboard.tutor.studentDetail.overview.stats.lastSession')}
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                            <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                                ...
                                                {t('dashboard.tutor.studentDetail.overview.contact.title')}
                                            </h3>
                                            <div className="space-y-3 text-sm">
                                                <div className="flex justify-between border-b border-gray-50 pb-2">
                                                    <p className="text-gray-500 font-medium">{t('dashboard.tutor.studentDetail.overview.contact.email')}</p>
                                                    <p className="text-gray-800 font-semibold">{student.email}</p>
                                                </div>
                                                <div className="flex justify-between border-b border-gray-50 pb-2">
                                                    <p className="text-gray-500 font-medium">{t('dashboard.tutor.studentDetail.overview.contact.phone')}</p>
                                                    <p className="text-gray-800 font-semibold">{student.contact.phone}</p>
                                                </div>
                                                <div className="flex justify-between">
                                                    <p className="text-gray-500 font-medium">{t('dashboard.tutor.studentDetail.overview.contact.joinedDate')}</p>
                                        ...
                                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                            <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                                ...
                                                {t('dashboard.tutor.studentDetail.overview.class.title')}
                                            </h3>
                                            <div className="space-y-3 text-sm">
                                                <div className="flex justify-between border-b border-gray-50 pb-2">
                                                    <p className="text-gray-500 font-medium">{t('dashboard.tutor.studentDetail.overview.class.className')}</p>
                                                    <p className="text-gray-800 font-semibold">{student.class.name}</p>
                                                </div>
                                                <div className="flex justify-between border-b border-gray-50 pb-2">
                                                    <p className="text-gray-500 font-medium">{t('dashboard.tutor.studentDetail.overview.class.instructor')}</p>
                                                    <p className="text-gray-800 font-semibold">{student.class.instructor}</p>
                                                </div>
                                                <div className="flex justify-between">
                                                    <p className="text-gray-500 font-medium">{t('dashboard.tutor.studentDetail.overview.class.schedule')}</p>
                                    ...
                                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl shadow-sm p-6">
                                        <h3 className="font-bold text-gray-800 mb-3">{t('dashboard.tutor.studentDetail.overview.payment.title')}</h3>
                                        <div className="flex flex-wrap items-center justify-between gap-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-3 h-3 rounded-full bg-green-500"></div>
                                                <p className="text-sm font-semibold text-gray-700">{t('dashboard.tutor.studentDetail.overview.payment.status')}: <span className="text-green-700">{student.payment.status}</span></p>
                                            </div>
                                            <div className="text-sm text-gray-600">
                                                <span className="font-medium">{t('dashboard.tutor.studentDetail.overview.payment.totalPaid')}:</span> <span className="font-bold text-gray-800">{student.payment.totalPaid}</span>
                                            </div>
                                            <div className="text-sm text-gray-600">
                                                <span className="font-medium">{t('dashboard.tutor.studentDetail.overview.payment.nextDue')}:</span> <span className="font-bold text-gray-800">{student.payment.nextDueDate}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                            ...
                                            {t('dashboard.tutor.studentDetail.schedule.upcomingSessions.title')}
                                        </h3>
                                                ...
                                                <div key={session.id} className="bg-blue-50 border border-blue-200 rounded-xl p-4 hover:shadow-sm transition-shadow">
                                                    <div className="flex flex-wrap justify-between items-start gap-2">
                                                        ...
                                                        <span className="px-3 py-1 bg-blue-600 text-white text-xs font-semibold rounded-full">
                                                            {t('dashboard.tutor.studentDetail.schedule.upcomingSessions.scheduled')}
                                                        </span>
                                                    </div>
                                                </div>
                                            ))}
                                    ...
                                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                        <div className="flex justify-between items-center mb-3">
                                            <h4 className="font-bold text-gray-800">{t('dashboard.tutor.studentDetail.schedule.attendance.title')}</h4>
                                    ...
                                    {/* Session History */}
                                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                        <h3 className="font-bold text-gray-800 mb-4">{t('dashboard.tutor.studentDetail.schedule.history.title')}</h3>
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-sm">
                                                <thead className="bg-gray-50 text-gray-600 font-semibold">
                                                    <tr>
                                                        <th className="p-3 text-left">{t('dashboard.tutor.studentDetail.schedule.history.date')}</th>
                                                        <th className="p-3 text-left">{t('dashboard.tutor.studentDetail.schedule.history.topic')}</th>
                                                        <th className="p-3 text-left">{t('dashboard.tutor.studentDetail.schedule.history.duration')}</th>
                                                        <th className="p-3 text-left">{t('dashboard.tutor.studentDetail.schedule.history.attendance')}</th>
                                                    </tr>
                                                </thead>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                            ...
                                            {t('dashboard.tutor.studentDetail.progress.courseProgress.title')}
                                        </h3>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl shadow-sm p-6">
                                            <h4 className="font-bold text-gray-800 mb-3">{t('dashboard.tutor.studentDetail.progress.metrics.averageTestScore')}</h4>
                                            <p className="text-4xl font-bold text-blue-600">{student.performance.averageScore}%</p>
                                            <p className="text-sm text-gray-600 mt-2">{t('dashboard.tutor.studentDetail.progress.metrics.basedOnTests', { count: student.performance.testScores.length })}</p>
                                        </div>
                                        <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-2xl shadow-sm p-6">
                                            <h4 className="font-bold text-gray-800 mb-3">{t('dashboard.tutor.studentDetail.progress.metrics.homeworkCompletion')}</h4>
                                            <p className="text-4xl font-bold text-green-600">{student.performance.homeworkCompletion}%</p>
                                            <p className="text-sm text-gray-600 mt-2">{t('dashboard.tutor.studentDetail.progress.metrics.excellentRate')}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                            <h4 className="font-bold text-green-700 mb-3">{t('dashboard.tutor.studentDetail.progress.strengths.title')}</h4>
                                        ...
                                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                            <h4 className="font-bold text-orange-700 mb-3">{t('dashboard.tutor.studentDetail.progress.weaknesses.title')}</h4>
                                            <div className="flex flex-wrap gap-2">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                            ...
                                            {t('dashboard.tutor.studentDetail.notes.title')}
                                        </h3>
                                        <textarea
                                            value={tutorNotes}
                                            ...
                                            placeholder={t('dashboard.tutor.studentDetail.notes.placeholder')}
                                        />
                                        <button className="mt-3 px-4 py-2 bg-[#0b6459] text-white rounded-lg hover:bg-[#084c43] transition-colors text-sm font-semibold">
                                            {t('dashboard.tutor.studentDetail.notes.saveButton')}
                                        </button>
                                    </div>

                                    {/* Communication History */}
                                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                        <h3 className="font-bold text-gray-800 mb-4">{t('dashboard.tutor.studentDetail.notes.communicationHistory.title')}</h3>
                                        <div className="space-y-3">
                                    ...
                                    <div className="bg-gradient-to-r from-[#0b6459]/10 to-teal-50 border border-[#0b6459]/30 rounded-2xl shadow-sm p-6">
                                        <h4 className="font-bold text-gray-800 mb-3">{t('dashboard.tutor.studentDetail.notes.quickActions.title')}</h4>
                                        <div className="flex flex-wrap gap-3">
                                            <button className="flex items-center gap-2 px-4 py-2 bg-[#0b6459] text-white rounded-lg hover:bg-[#084c43] transition-colors text-sm font-semibold">
                                                <HiChat className="w-4 h-4" />
                                                {t('dashboard.tutor.studentDetail.notes.quickActions.sendMessage')}
                                            </button>
                                            <button className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm font-semibold">
                                                <HiCalendar className="w-4 h-4" />
                                                {t('dashboard.tutor.studentDetail.notes.quickActions.scheduleSession')}
                                            </button>
                                        </div>
                                        <p className="text-xs text-gray-600 mt-3">{t('dashboard.tutor.studentDetail.notes.quickActions.lastContacted')}: 2025-11-19</p>
                                    </div>
                                </div>
                            )}
                        </div>
