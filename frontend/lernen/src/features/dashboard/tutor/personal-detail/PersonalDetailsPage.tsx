...
import type { SocialLink, Language as ApiLanguage, Subject as ApiSubject, EducationItem, ExperienceItem, CertificationItem, Country, Language, Subject } from "../../../../types/api";
        ...
        const fetchOptions = async () => {
            try {
                ...
                const languages: Language[] = [
                    { code: 'en', name: 'English', isNative: false },
                    { code: 'vi', name: 'Vietnamese', isNative: true },
                    { code: 'ja', name: 'Japanese', isNative: false },
                ];
                            ...
                            {activeTab === "Zoom Info" && (
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 mt-1">
                                            Connect your Zoom account to enable video sessions with students
                                        </p>
                                    </div>
                                    ...
                                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className="text-lg font-semibold text-gray-800">Zoom Account</h3>
                                                    <p className="text-sm text-gray-600">
                                                        {profile?.zoomConnected ? 
                                                            "Connected - Ready to host sessions" : 
                                                            "Not connected - Connect to start hosting video sessions"
                                                        }
                                                    </p>
                                                </div>
                                            </div>
                                            <button
                                                onClick={async () => {
                                                    ...
                                                    try {
                                                        const response = await classService.zoomAuthorize(authState.user.id);
                                                        ...
                                                        setToast({ message: 'Failed to connect to Zoom', type: 'error' });
                                                    } finally {
                                            ...
                                            >
                                                {isZoomLoading ? 'Connecting...' : (profile?.zoomConnected ? 'Reconnect Zoom' : 'Connect Zoom')}
                                            </button>
                                        </div>
                                        ...
                                        {profile?.zoomConnected && (
                                            <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-medium text-green-800">Zoom account connected successfully</span>
                                                </div>
                                                <p className="text-sm text-green-700 mt-1">
                                                    You can now host video sessions with your students through Zoom.
                                                </p>
                                            </div>
                                        )}
                                    </div>
