...
const SessionDetailModal: React.FC<SessionDetailModalProps> = ({ booking, position, onClose }) => {
    ...
    const startTime = booking.date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
    const endDate = new Date(booking.date.getTime() + booking.durationHours * 60 * 60 * 1000);
    ...
    return (
        <div
            ref={modalRef}
            className={`fixed z-30 w-80 bg-white rounded-xl shadow-2xl border border-gray-200/80 p-5 transition-all duration-200 ease-out origin-top-left ${mounted ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`}
