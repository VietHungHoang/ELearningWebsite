...
const AssignQuizModal: React.FC<AssignQuizModalProps> = ({ isOpen, onClose, onAssign }) => {
    ...
    const handleAssign = () => {
        if (selectedQuiz && dueDate) {
            ...
            alert(t('quiz.assign.validation'));
        }
    };
        ...
        <div className={`fixed inset-0 z-[60] flex items-center justify-center p-4 transition-opacity duration-300 ${isOpen ? 'bg-black/30 opacity-100' : 'opacity-0'}`} onClick={onClose}>
            <div className={`bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all duration-300 ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`} onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-start mb-6">
                    <h2 className="text-lg font-bold text-gray-800">{t('quiz.assign.title')}</h2>
                ...
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('quiz.assign.selectLabel')}</label>
                        <select
                            value={selectedQuiz}
                        ...
                        >
                            <option value="">{t('quiz.assign.selectPlaceholder')}</option>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('quiz.assign.dueDate')}</label>
                        <input
                            type="date"
                            value={dueDate}
                ...
                <div className="flex justify-end items-center gap-3 mt-8">
                    <button onClick={onClose} className="px-5 py-2.5 text-sm font-semibold bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">{t('common.cancel')}</button>
                    <button
