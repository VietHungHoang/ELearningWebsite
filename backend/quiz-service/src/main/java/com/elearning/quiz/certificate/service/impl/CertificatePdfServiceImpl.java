package com.elearning.quiz.certificate.service.impl;
...
public class CertificatePdfServiceImpl implements CertificatePdfService {
    ...
    public byte[] generateCourseraStyleCertificate(CertificatePreviewRequest request) {
        String organizationName = request.getOrganizationName() != null ? 
            request.getOrganizationName() : "PROFESSIONAL E-LEARNING INSTITUTE";
        ...
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            createBalancedMainContent(document, request, width, height, serifBold, serifItalic, serif, sansSerif);
    ...
    private void createBalancedMainContent(Document document, CertificatePreviewRequest request,
        ...
        Paragraph achievement1 = new Paragraph("has successfully completed the comprehensive course of study and")
            .setTextAlignment(TextAlignment.CENTER)
            .setFont(serif)
            .setFontSize(14)
            .setFontColor(DARK_GRAY)
        ...
        String courseDetails = String.format("Authorized by %s and delivered through our accredited learning platform", 
            request.getOrganizationName() != null ? request.getOrganizationName() : "Professional E-Learning Institute");
    ...
    private void createRightSidebar(PdfDocument pdf, Document document, CertificatePreviewRequest request,
        ...
        createModernInfoPanel(document, request, sidebarX + sidebarWidth/2, sidebarY - 140f, sidebarWidth - 40f, sansSerif, sansSerifBold);
        
        // QR code at bottom - adjusted for tapered container
        addVerificationQRCode(pdf, document, request, sidebarX + 65, 120f); // Moved up to avoid taper
    }
    ...
    private void createModernSidebarContainer(PdfDocument pdf, float x, float y, float width, float height) {
        ...
        float taperWidth = 30f; // Width of the taper at bottom
    ...
    private void addModernInfoItem(Document document, String label, String value,
            float centerX, float y, float width, PdfFont labelFont, PdfFont valueFont, DeviceRgb valueColor) {
    ...
    private void addVerificationQRCode(PdfDocument pdf, Document document, 
            CertificatePreviewRequest request, float x, float y) {
        
        try {
            String verificationUrl = String.format("https://verify.elearning.com/certificate/%s", 
                request.getCertificateId() != null ? request.getCertificateId() : UUID.randomUUID().toString());
            ...
            Paragraph qrLabel = new Paragraph("Scan to Verify")
                ...
                .setFontSize(8)
                .setFontColor(MEDIUM_GRAY);
            qrLabel.setFixedPosition(1, x - 10, y - 15, 70);
    ...
    private void createBalancedSignatureArea(Document document, CertificatePreviewRequest request,
        ...
        Paragraph issueDate = new Paragraph("Issued on " + LocalDate.now().format(DateTimeFormatter.ofPattern("MMMM dd, yyyy")))
            .setTextAlignment(TextAlignment.CENTER)
            .setFont(sansSerif)
            .setFontSize(11)
            .setFontColor(MEDIUM_GRAY);
    ...
    private void createProfessionalFooter(Document document, String organizationName,
        ...
        Paragraph accreditation = new Paragraph("This certificate represents successful completion of a rigorous professional development program")
            .setTextAlignment(TextAlignment.CENTER)
            .setFont(sansSerif)
            .setFontSize(9)
            .setFontColor(MEDIUM_GRAY);
        ...
        Paragraph contact = new Paragraph("www.professionalelearning.edu  |  certificates@peli.edu  |  +1 (555) 123-4567")
            .setTextAlignment(TextAlignment.CENTER)
            .setFont(sansSerif)
            .setFontSize(8)
            .setFontColor(LIGHT_GRAY);
