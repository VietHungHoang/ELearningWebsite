...
import com.elearning.tutorservice.dto.zoom.request.ZoomOAuthCallbackRequest;
...
public class ZoomOAuthController {
    ...
    public ResponseEntity<ApiResponse<Void>> callback(@RequestBody ZoomOAuthCallbackRequest request) {
        log.info("Received Zoom OAuth callback with state: {}", request.getState());
        zoomOAuthService.processOAuthCallback(request.getCode(), request.getState());
