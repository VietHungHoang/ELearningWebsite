...
public interface TutorOnboardingRepository extends JpaRepository<TutorOnboarding, UUID> {
       ...
       @Query("SELECT COUNT(t) FROM TutorOnboarding t WHERE t.status = :status AND DATE(t.createdAt) BETWEEN :startDate AND :endDate")
       long countByStatusAndCreatedAtBetween(@Param("status") OnboardingStatus status,
